...
### 4. 输出结构（FR-014 分层结构 - 推荐卡片优先）

你必须按以下格式输出，**推荐卡片放在最前面**：
...
| **推荐官能团** | [具体官能团名称] | 高/中/低 |
| **推荐试剂** | [具体试剂名称] | 高/中/低 |
| **推荐官能化程度** | [X.X - Y.Y wt%] | 高/中/低 |
| **预期改善效果** | [具体效果描述] | 高/中/低 |

> **最佳参考样本**: 方案 X（[官能团]官能化，[程度]）

## 推荐理由

[1-2 段话简要说明推荐的核心理由，需要引用具体数据]
...
📊 **数据支撑**: 本推荐基于 X 个样本数据，来自 Y 篇独立文献
⚠️ **置信度说明**: [说明数据充分性/外推情况]
```

### 5. 推荐置信度判定规则
- **高置信度**：≥5 个数据点支撑，且结论一致
- **中置信度**：3-4 个数据点，或存在轻微数据冲突
- **低置信度**：<3 个数据点，或推荐基于外推
...
请基于以上样本信息，生成一个带有**明确推荐结论**的综合性回答：

1. **必须首先给出推荐方案**（官能团 + 试剂 + 程度范围 + 预期效果）
2. 综合多个样本的数据得出推荐理由（不是简单罗列）
3. 使用 Markdown 表格展示推荐参数
4. 每个数据点标注来源（使用"方案 X"格式）
5. 回答长度 600-800 字

**重要：你必须给出一个明确的推荐结论，即使数据存在分歧，也要基于现有数据给出最佳推荐，并说明理由和置信度。**"""
