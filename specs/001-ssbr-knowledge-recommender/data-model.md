...
> **架构变更**：解读结果不再存储于 Excel 列，而是采用文件系统存储，详见 `interpretation-storage-design.md`。

解读文档存储路径：`/dataset/interpretations/{样本ID}/`

| 文件名 | 对应表征列 | 说明 |
|--------|-----------|------|
| `nmr.md` | I列（核磁谱图） | ¹H NMR 标准化解读 |
| `tem.md` | J列（微相分离图片表征） | TEM 形貌标准化解读 |
| `mechanical.md` | K列（核心力学图谱） | 应力-应变/Payne/DMA 解读（支持多类型） |
| `dsc.md` | L列（DSC谱图） | DSC 热分析标准化解读 |
| `summary.md` | - | 综合摘要（用于 RAG 向量检索）|

### Excel 新增字段

| 字段名 | 列 | 数据类型 | 说明 |
|--------|---|----------|------|
| 解读状态 | X | String | 标记解读完成度，如 `0/4`、`4/4+S` |
