...
基于S列内容进行关键词匹配，判断该样本的力学图谱类型：

### 识别规则（按优先级从高到低）

| 优先级 | 匹配条件 | 识别结果 | 调用Skill |
|--------|----------|----------|-----------|
| 1 | S列包含 `Payne` 或 `G'-应变` 或 `储能模量` | Payne效应曲线 | `ssbr-payne-interpretation` |
| 2 | S列包含 `DMA` 或 `tan δ` 或 `tanδ` 或 `损耗因子` | DMA曲线 | `ssbr-dma-interpretation` |
| 3 | S列包含 `应力-应变` 或 `应力 - 应变` 或 `拉伸` | 应力-应变曲线 | `ssbr-stress-strain-interpretation` |
| 4 | 以上均不匹配 | **需用户确认** | 见下文 |

### 混合图谱处理规则

若S列同时包含多种图谱类型（如 `应力-应变曲线及tan δ曲线`），按以下规则处理：

1. **识别所有图谱类型**：列出S列中包含的所有图谱类型
2. **询问用户优先解读哪种**：输出选择提示
...
【调度结果】
- 样本：SSBR-006
- S列内容：文献 Fig.6 Payne效应曲线及Fig.9 DMA曲线
...
| 错误场景 | 输出信息 |
|----------|----------|
| S列为空或 `-` | 「错误：该样本的S列（核心力学图谱）为空，无法进行力学图谱解读」 |
| 无法识别图谱类型 | 「提示：无法自动识别该样本的力学图谱类型，S列内容为：{内容}。请手动指定要解读的图谱类型（应力-应变 / Payne / DMA）」 |
...
| Skill名称 | 职责 | 调用条件 |
|-----------|------|----------|
| `ssbr-stress-strain-interpretation` | 应力-应变曲线解读 | S列含"应力-应变"关键词 |
| `ssbr-payne-interpretation` | Payne效应曲线解读 | S列含"Payne"关键词 |
| `ssbr-dma-interpretation` | DMA温度扫描曲线解读 | S列含"tan δ"或"DMA"关键词 |
