---
sample_id: SSBR-018
doi: 10.1021/acs.iecr.8b05738
polymer_type: null
data_completeness:
  mechanical: true
  dsc: true
  nmr: true
  tem: true
created_at: '2026-03-21'
updated_at: '2026-03-24'
functionalization:
  type: chain_end_or_additive
  reagent: DPE-NMe2
  grafting_group: 二甲基氨基（链端）
  grafting_group_smiles: CN(C)
  core_functional_group_smiles: N
  core_functional_group_name: 氨基
  degree: 0.09 wt%
polymer_fingerprint: '--------CN(C)---0.09wt%'
---

# SSBR-018 综合档案

## 一句话总结

链端氨基官能化 SSBR（α-SSBR），通过氨基-羟基氢键作用显著改善白炭黑分散性，在四种官能化策略中展现出最佳的综合轮胎性能平衡（高抓地力 + 低滚阻）。

---

## 核心性能特点

### 【白炭黑分散性最优】 【优秀】

α-SSBR/silica 复合材料表现出所有样本中**最小的 Payne 效应（ΔG'）**：
- 白炭黑团聚体尺寸从 ~200 nm 降至 90-125 nm
- TXM 3D 成像确认 <100 nm 团聚体比例显著提高
- 氨基-羟基氢键有效传递剪切力，促进团聚体破碎

### 【轮胎"魔三角"性能平衡】 【优秀】

在四种 SSBR 变体中，α-SSBR 展现出最佳综合性能：
- **高 tan δ(0°C)**：优异湿地抓地力
- **低 tan δ(60°C)**：低滚动阻力
- **高力学强度**：改善的拉伸、撕裂性能

> "α-SSBR/silica composite showed higher static mechanical properties, better wet-skid resistance, and lower loss factors, making it an appropriate material towards the manufacturing of high-performance green tire."

### 【界面相互作用增强】 【良好】

AFM 表征确认界面改善：
- 紧密结合橡胶层（TBR）厚度：10.8 nm vs 原始 7.6 nm（+42%）
- 松散结合橡胶层（LBR）厚度：12.5 nm vs 原始 8.7 nm（+44%）
- 证明氨基官能团有效增强了橡胶-白炭黑界面

---


---

## 适用场景

- ✅ 二氧化硅分散

---

## 关键性能指标

| 类别 | 指标 | 数值 | 单位 | 评价 |
|------|------|------|------|------|
| 力学性能 | 100%定伸应力 | - | MPa | - |
| 力学性能 | 300%定伸应力 | - | MPa | - |
| 力学性能 | 拉伸强度 | 优于原始SSBR | MPa | - |
| 力学性能 | 断裂伸长率 | 优于原始SSBR | % | - |
| 热学性能 | Tg | -17.4 | ℃ | - |

---

## 解读文档完整性

| 文档类型 | 状态 | 备注 |
|----------|------|------|
| mechanical.md | ✓ | 已生成 |
| dsc.md | ✓ | 已生成 |
| nmr.md | ✓ | 已生成 |
| tem.md | ✓ | 已生成 |

---

## 文献来源

- **DOI**: 10.1021/acs.iecr.8b05738
- **引文**: Sun C, et al. [J]. Ind. Eng. Chem. Res. 2019, 58.

---


## 曲线数据概览 (v2.0)

本样本已完成全范围曲线数据提取，支持高精度分析和趋势外推。

### 应力-应变曲线
- **数据点数**: 0
- **应变范围**: 0-0%
- **应力范围**: 0-0 MPa
- **平均置信度**: N/A
- **交叉验证**: poor

### DMA tan δ-温度曲线
- **数据点数**: 0
- **温度范围**: -80-80℃
- **tan δ 范围**: 0-1
- **平均置信度**: N/A
- **交叉验证**: poor
- **关键特征值**:
  - Tg (tan δ 峰值): -17.4℃

### Payne G'-应变曲线
- **数据点数**: 0
- **应变范围**: 0.1-100% (对数间隔)
- **G' 范围**: 0-3 MPa
- **平均置信度**: N/A
- **交叉验证**: poor
- **关键特征值**:

### DSC 热流曲线
- **数据点数**: 0
- **温度范围**: -80-100℃
- **平均置信度**: N/A
- **交叉验证**: acceptable
- **玻璃化转变**:
  - Tg midpoint: -17.4℃
---

*本综合档案由 `ssbr-summary-generator` 脚本合并生成，保留了人工撰写的解读内容，用于 RAG 语义检索。*